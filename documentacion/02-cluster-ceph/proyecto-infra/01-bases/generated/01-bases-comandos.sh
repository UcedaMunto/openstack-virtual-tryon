#!/usr/bin/env bash
set -euo pipefail

# Archivo generado por create-kvm-vm.sh --comandos
# Ejecuta este archivo cuando quieras crear las VMs manualmente.

# 2026-04-30 23:17:51 ------------------------------------------------
# VM: ceph-admin
# Paso 1: validar imagen base
test -f /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 || { echo '[ERROR] Falta imagen base: /var/lib/libvirt/images/ubuntu-22.04-base.qcow2'; exit 1; }

# Paso 2: generar disco del sistema (si no existe)
if [[ ! -f /var/lib/libvirt/images/ceph-admin.qcow2 ]]; then
  sudo qemu-img create -f qcow2 -b /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 -F qcow2 /var/lib/libvirt/images/ceph-admin.qcow2 20G
fi

# Paso 4: crear cloud-init user-data
cat > /tmp/user-data-ceph-admin.yaml <<'EOF'
#cloud-config
hostname: ceph-admin
manage_etc_hosts: false
timezone: America/El_Salvador
users:
  - name: admin
    primary_group: admin
    sudo: ALL=(ALL) NOPASSWD:ALL
    shell: /bin/bash
    lock_passwd: false
    passwd: $6$pm9w4unG8C2HuxYI$ylqwre00LVenw7wz4l76w4h6Vj6ET2p/z5z6MwDKmlbfB3b5Z264x73cWRegG78OvFnUiMDN4m1WqGV1v4zYK.
    ssh_authorized_keys:
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
chpasswd:
  expire: false
ssh_pwauth: true
write_files:
  - path: /etc/hosts
    owner: root:root
    permissions: '0644'
    content: |
      127.0.0.1 localhost
      127.0.1.1 ceph-admin
      192.168.3.10 ceph-admin
      192.168.3.11 ceph-mon
      192.168.3.12 ceph-osd1
      192.168.3.13 ceph-osd2
      192.168.3.14 ceph-osd3
  - path: /home/admin/.ssh/id_rsa
    owner: admin:admin
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /home/admin/.ssh/id_rsa.pub
    owner: admin:admin
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /home/admin/.ssh/authorized_keys
    owner: admin:admin
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/id_rsa
    owner: root:root
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /root/.ssh/id_rsa.pub
    owner: root:root
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/authorized_keys
    owner: root:root
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a

package_update: true
packages:
  - chrony
  - openssh-server
  - curl
  - qemu-guest-agent
runcmd:
  - timedatectl set-timezone America/El_Salvador || true
  - sed -i '/^pool /d;/^server /d' /etc/chrony/chrony.conf
  - printf 'server ntp.ues.edu.sv iburst\n' >> /etc/chrony/chrony.conf
  - systemctl enable --now chrony
  - systemctl enable --now ssh
  - systemctl enable --now qemu-guest-agent
  - chmod 700 /home/admin/.ssh
  - chmod 600 /home/admin/.ssh/id_rsa /home/admin/.ssh/authorized_keys
  - chmod 644 /home/admin/.ssh/id_rsa.pub
  - chown -R admin:admin /home/admin/.ssh
  - chmod 700 /root/.ssh
  - chmod 600 /root/.ssh/id_rsa /root/.ssh/authorized_keys
  - chmod 644 /root/.ssh/id_rsa.pub

EOF

# Paso 5: crear cloud-init network-config
cat > /tmp/network-config-ceph-admin.yaml <<'EOF'
version: 2
ethernets:
  enp1s0:
    match:
      macaddress: 52:54:00:cc:dd:10
    set-name: enp1s0
    dhcp4: false
    addresses: [192.168.3.10/24]
    routes:
      - to: default
        via: 192.168.3.1
    nameservers:
      addresses: [8.8.8.8,8.8.4.4]
EOF

# Paso 6: crear VM con virt-install
if ! sudo virsh dominfo ceph-admin >/dev/null 2>&1; then
  sudo virt-install \
    --name ceph-admin \n    --ram 2048 \n    --vcpus 2 \n    --disk path=/var/lib/libvirt/images/ceph-admin.qcow2\,format=qcow2 \n    --network network=net-192-168-3\,model=virtio\,mac=52:54:00:cc:dd:10 \n    --os-variant ubuntu22.04 \n    --cloud-init user-data=/tmp/user-data-ceph-admin.yaml\,network-config=/tmp/network-config-ceph-admin.yaml \n    --noautoconsole \n    --import
else
  echo \[WARN\]\ La\ VM\ ceph-admin\ ya\ existe\ en\ libvirt.\ Se\ omite\ creacion.
fi

# 2026-04-30 23:18:08 ------------------------------------------------
# VM: ceph-admin
# Paso 1: validar imagen base
test -f /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 || { echo '[ERROR] Falta imagen base: /var/lib/libvirt/images/ubuntu-22.04-base.qcow2'; exit 1; }

# Paso 2: generar disco del sistema (si no existe)
if [[ ! -f /var/lib/libvirt/images/ceph-admin.qcow2 ]]; then
  sudo qemu-img create -f qcow2 -b /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 -F qcow2 /var/lib/libvirt/images/ceph-admin.qcow2 20G
fi

# Paso 4: crear cloud-init user-data
cat > /tmp/user-data-ceph-admin.yaml <<'EOF'
#cloud-config
hostname: ceph-admin
manage_etc_hosts: false
timezone: America/El_Salvador
users:
  - name: admin
    primary_group: admin
    sudo: ALL=(ALL) NOPASSWD:ALL
    shell: /bin/bash
    lock_passwd: false
    passwd: $6$sZ76lfRUpr0LZc03$wrGmN/J18W58TuabtqORGovCw5k11kbk3rkMJzTeAnPOkQtthQSb6NAnSk9npKU8QI1lFVGS1XZPjc/vhetwa/
    ssh_authorized_keys:
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
chpasswd:
  expire: false
ssh_pwauth: true
write_files:
  - path: /etc/hosts
    owner: root:root
    permissions: '0644'
    content: |
      127.0.0.1 localhost
      127.0.1.1 ceph-admin
      192.168.3.10 ceph-admin
      192.168.3.11 ceph-mon
      192.168.3.12 ceph-osd1
      192.168.3.13 ceph-osd2
      192.168.3.14 ceph-osd3
  - path: /home/admin/.ssh/id_rsa
    owner: admin:admin
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /home/admin/.ssh/id_rsa.pub
    owner: admin:admin
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /home/admin/.ssh/authorized_keys
    owner: admin:admin
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/id_rsa
    owner: root:root
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /root/.ssh/id_rsa.pub
    owner: root:root
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/authorized_keys
    owner: root:root
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a

package_update: true
packages:
  - chrony
  - openssh-server
  - curl
  - qemu-guest-agent
runcmd:
  - timedatectl set-timezone America/El_Salvador || true
  - sed -i '/^pool /d;/^server /d' /etc/chrony/chrony.conf
  - printf 'server ntp.ues.edu.sv iburst\n' >> /etc/chrony/chrony.conf
  - systemctl enable --now chrony
  - systemctl enable --now ssh
  - systemctl enable --now qemu-guest-agent
  - chmod 700 /home/admin/.ssh
  - chmod 600 /home/admin/.ssh/id_rsa /home/admin/.ssh/authorized_keys
  - chmod 644 /home/admin/.ssh/id_rsa.pub
  - chown -R admin:admin /home/admin/.ssh
  - chmod 700 /root/.ssh
  - chmod 600 /root/.ssh/id_rsa /root/.ssh/authorized_keys
  - chmod 644 /root/.ssh/id_rsa.pub

EOF

# Paso 5: crear cloud-init network-config
cat > /tmp/network-config-ceph-admin.yaml <<'EOF'
version: 2
ethernets:
  enp1s0:
    match:
      macaddress: 52:54:00:cc:dd:10
    set-name: enp1s0
    dhcp4: false
    addresses: [192.168.3.10/24]
    routes:
      - to: default
        via: 192.168.3.1
    nameservers:
      addresses: [8.8.8.8,8.8.4.4]
EOF

# Paso 6: crear VM con virt-install
if ! sudo virsh dominfo ceph-admin >/dev/null 2>&1; then
  sudo virt-install \
    --name ceph-admin \n    --ram 2048 \n    --vcpus 2 \n    --disk path=/var/lib/libvirt/images/ceph-admin.qcow2\,format=qcow2 \n    --network network=net-192-168-3\,model=virtio\,mac=52:54:00:cc:dd:10 \n    --os-variant ubuntu22.04 \n    --cloud-init user-data=/tmp/user-data-ceph-admin.yaml\,network-config=/tmp/network-config-ceph-admin.yaml \n    --noautoconsole \n    --import
else
  echo \[WARN\]\ La\ VM\ ceph-admin\ ya\ existe\ en\ libvirt.\ Se\ omite\ creacion.
fi

# 2026-04-30 23:19:16 ------------------------------------------------
# VM: ceph-admin
# Paso 1: validar imagen base
test -f /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 || { echo '[ERROR] Falta imagen base: /var/lib/libvirt/images/ubuntu-22.04-base.qcow2'; exit 1; }

# Paso 2: generar disco del sistema (si no existe)
if [[ ! -f /var/lib/libvirt/images/ceph-admin.qcow2 ]]; then
  sudo qemu-img create -f qcow2 -b /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 -F qcow2 /var/lib/libvirt/images/ceph-admin.qcow2 20G
fi

# Paso 4: crear cloud-init user-data
cat > /tmp/user-data-ceph-admin.yaml <<'EOF'
#cloud-config
hostname: ceph-admin
manage_etc_hosts: false
timezone: America/El_Salvador
users:
  - name: admin
    primary_group: admin
    sudo: ALL=(ALL) NOPASSWD:ALL
    shell: /bin/bash
    lock_passwd: false
    passwd: $6$spb7f1todBAHRK9Q$f7uiXPIbKL1UY77GJS1lx9FhYdTvSAK.4I1ZqozHiUUjyxG9Pm0c071CHRD8E4ABXa78JDLseacVbE/5PWio./
    ssh_authorized_keys:
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
chpasswd:
  expire: false
ssh_pwauth: true
write_files:
  - path: /etc/hosts
    owner: root:root
    permissions: '0644'
    content: |
      127.0.0.1 localhost
      127.0.1.1 ceph-admin
      192.168.3.10 ceph-admin
      192.168.3.11 ceph-mon
      192.168.3.12 ceph-osd1
      192.168.3.13 ceph-osd2
      192.168.3.14 ceph-osd3
  - path: /home/admin/.ssh/id_rsa
    owner: admin:admin
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /home/admin/.ssh/id_rsa.pub
    owner: admin:admin
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /home/admin/.ssh/authorized_keys
    owner: admin:admin
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/id_rsa
    owner: root:root
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /root/.ssh/id_rsa.pub
    owner: root:root
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/authorized_keys
    owner: root:root
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a

package_update: true
packages:
  - chrony
  - openssh-server
  - curl
  - qemu-guest-agent
runcmd:
  - timedatectl set-timezone America/El_Salvador || true
  - sed -i '/^pool /d;/^server /d' /etc/chrony/chrony.conf
  - printf 'server ntp.ues.edu.sv iburst\n' >> /etc/chrony/chrony.conf
  - systemctl enable --now chrony
  - systemctl enable --now ssh
  - systemctl enable --now qemu-guest-agent
  - chmod 700 /home/admin/.ssh
  - chmod 600 /home/admin/.ssh/id_rsa /home/admin/.ssh/authorized_keys
  - chmod 644 /home/admin/.ssh/id_rsa.pub
  - chown -R admin:admin /home/admin/.ssh
  - chmod 700 /root/.ssh
  - chmod 600 /root/.ssh/id_rsa /root/.ssh/authorized_keys
  - chmod 644 /root/.ssh/id_rsa.pub

EOF

# Paso 5: crear cloud-init network-config
cat > /tmp/network-config-ceph-admin.yaml <<'EOF'
version: 2
ethernets:
  enp1s0:
    match:
      macaddress: 52:54:00:cc:dd:10
    set-name: enp1s0
    dhcp4: false
    addresses: [192.168.3.10/24]
    routes:
      - to: default
        via: 192.168.3.1
    nameservers:
      addresses: [8.8.8.8,8.8.4.4]
EOF

# Paso 6: crear VM con virt-install
if ! sudo virsh dominfo ceph-admin >/dev/null 2>&1; then
  sudo virt-install \
    --name ceph-admin \n    --ram 2048 \n    --vcpus 2 \n    --disk path=/var/lib/libvirt/images/ceph-admin.qcow2\,format=qcow2 \n    --network network=net-192-168-3\,model=virtio\,mac=52:54:00:cc:dd:10 \n    --os-variant ubuntu22.04 \n    --cloud-init user-data=/tmp/user-data-ceph-admin.yaml\,network-config=/tmp/network-config-ceph-admin.yaml \n    --noautoconsole \n    --import
else
  echo \[WARN\]\ La\ VM\ ceph-admin\ ya\ existe\ en\ libvirt.\ Se\ omite\ creacion.
fi

# 2026-04-30 23:19:50 ------------------------------------------------
# VM: ceph-admin
# Paso 1: validar imagen base
test -f /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 || { echo '[ERROR] Falta imagen base: /var/lib/libvirt/images/ubuntu-22.04-base.qcow2'; exit 1; }

# Paso 2: generar disco del sistema (si no existe)
if [[ ! -f /var/lib/libvirt/images/ceph-admin.qcow2 ]]; then
  sudo qemu-img create -f qcow2 -b /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 -F qcow2 /var/lib/libvirt/images/ceph-admin.qcow2 20G
fi

# Paso 4: crear cloud-init user-data
cat > /tmp/user-data-ceph-admin.yaml <<'EOF'
#cloud-config
hostname: ceph-admin
manage_etc_hosts: false
timezone: America/El_Salvador
users:
  - name: admin
    primary_group: admin
    sudo: ALL=(ALL) NOPASSWD:ALL
    shell: /bin/bash
    lock_passwd: false
    passwd: $6$y4jc19YnzwTBgvsL$IY3bhVG0CTua374lfppldY8hNRZoLnV9E2BrOcQKqez4lYbte0n35aD1h2ssP1V.HXyn0Bt9IzkLKbu/2KzqS1
    ssh_authorized_keys:
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
chpasswd:
  expire: false
ssh_pwauth: true
write_files:
  - path: /etc/hosts
    owner: root:root
    permissions: '0644'
    content: |
      127.0.0.1 localhost
      127.0.1.1 ceph-admin
      192.168.3.10 ceph-admin
      192.168.3.11 ceph-mon
      192.168.3.12 ceph-osd1
      192.168.3.13 ceph-osd2
      192.168.3.14 ceph-osd3
  - path: /home/admin/.ssh/id_rsa
    owner: admin:admin
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /home/admin/.ssh/id_rsa.pub
    owner: admin:admin
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /home/admin/.ssh/authorized_keys
    owner: admin:admin
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/id_rsa
    owner: root:root
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /root/.ssh/id_rsa.pub
    owner: root:root
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/authorized_keys
    owner: root:root
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a

package_update: true
packages:
  - chrony
  - openssh-server
  - curl
  - qemu-guest-agent
runcmd:
  - timedatectl set-timezone America/El_Salvador || true
  - sed -i '/^pool /d;/^server /d' /etc/chrony/chrony.conf
  - printf 'server ntp.ues.edu.sv iburst\n' >> /etc/chrony/chrony.conf
  - systemctl enable --now chrony
  - systemctl enable --now ssh
  - systemctl enable --now qemu-guest-agent
  - chmod 700 /home/admin/.ssh
  - chmod 600 /home/admin/.ssh/id_rsa /home/admin/.ssh/authorized_keys
  - chmod 644 /home/admin/.ssh/id_rsa.pub
  - chown -R admin:admin /home/admin/.ssh
  - chmod 700 /root/.ssh
  - chmod 600 /root/.ssh/id_rsa /root/.ssh/authorized_keys
  - chmod 644 /root/.ssh/id_rsa.pub

EOF

# Paso 5: crear cloud-init network-config
cat > /tmp/network-config-ceph-admin.yaml <<'EOF'
version: 2
ethernets:
  enp1s0:
    match:
      macaddress: 52:54:00:cc:dd:10
    set-name: enp1s0
    dhcp4: false
    addresses: [192.168.3.10/24]
    routes:
      - to: default
        via: 192.168.3.1
    nameservers:
      addresses: [8.8.8.8,8.8.4.4]
EOF

# Paso 6: crear VM con virt-install
if ! sudo virsh dominfo ceph-admin >/dev/null 2>&1; then
  sudo virt-install \
    --name ceph-admin \n    --ram 2048 \n    --vcpus 2 \n    --disk path=/var/lib/libvirt/images/ceph-admin.qcow2\,format=qcow2 \n    --network network=net-192-168-3\,model=virtio\,mac=52:54:00:cc:dd:10 \n    --os-variant ubuntu22.04 \n    --cloud-init user-data=/tmp/user-data-ceph-admin.yaml\,network-config=/tmp/network-config-ceph-admin.yaml \n    --noautoconsole \n    --import
else
  echo \[WARN\]\ La\ VM\ ceph-admin\ ya\ existe\ en\ libvirt.\ Se\ omite\ creacion.
fi

# 2026-04-30 23:50:50 ------------------------------------------------
# VM: ceph-admin
# Paso 1: validar imagen base
test -f /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 || { echo '[ERROR] Falta imagen base: /var/lib/libvirt/images/ubuntu-22.04-base.qcow2'; exit 1; }

# Paso 2: generar disco del sistema (si no existe)
if [[ ! -f /var/lib/libvirt/images/ceph-admin.qcow2 ]]; then
  sudo qemu-img create -f qcow2 -b /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 -F qcow2 /var/lib/libvirt/images/ceph-admin.qcow2 20G
fi

# Paso 4: crear cloud-init user-data
cat > /tmp/user-data-ceph-admin.yaml <<'EOF'
#cloud-config
hostname: ceph-admin
manage_etc_hosts: false
timezone: America/El_Salvador
users:
  - name: admin
    primary_group: admin
    sudo: ALL=(ALL) NOPASSWD:ALL
    shell: /bin/bash
    lock_passwd: false
    passwd: $6$yWq5ktmnBg2A1NMm$/x0C0E2Bt1NWqmAQjgG56IR8BmfBv.9bHEXBrpVgYrKhh3oQN3E3owHNsGhSPw4JZ0vxqZ4FG8Np/k9WeNnkG0
    ssh_authorized_keys:
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
chpasswd:
  expire: false
ssh_pwauth: true
write_files:
  - path: /etc/hosts
    owner: root:root
    permissions: '0644'
    content: |
      127.0.0.1 localhost
      127.0.1.1 ceph-admin
      192.168.3.10 ceph-admin
      192.168.3.11 ceph-mon
      192.168.3.12 ceph-osd1
      192.168.3.13 ceph-osd2
      192.168.3.14 ceph-osd3
  - path: /home/admin/.ssh/id_rsa
    owner: admin:admin
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /home/admin/.ssh/id_rsa.pub
    owner: admin:admin
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /home/admin/.ssh/authorized_keys
    owner: admin:admin
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/id_rsa
    owner: root:root
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /root/.ssh/id_rsa.pub
    owner: root:root
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/authorized_keys
    owner: root:root
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a

package_update: true
packages:
  - chrony
  - openssh-server
  - curl
  - qemu-guest-agent
runcmd:
  - timedatectl set-timezone America/El_Salvador || true
  - sed -i '/^pool /d;/^server /d' /etc/chrony/chrony.conf
  - printf 'server ntp.ues.edu.sv iburst\n' >> /etc/chrony/chrony.conf
  - systemctl enable --now chrony
  - systemctl enable --now ssh
  - systemctl enable --now qemu-guest-agent
  - chmod 700 /home/admin/.ssh
  - chmod 600 /home/admin/.ssh/id_rsa /home/admin/.ssh/authorized_keys
  - chmod 644 /home/admin/.ssh/id_rsa.pub
  - chown -R admin:admin /home/admin/.ssh
  - chmod 700 /root/.ssh
  - chmod 600 /root/.ssh/id_rsa /root/.ssh/authorized_keys
  - chmod 644 /root/.ssh/id_rsa.pub

EOF

# Paso 5: crear cloud-init network-config
cat > /tmp/network-config-ceph-admin.yaml <<'EOF'
version: 2
ethernets:
  enp1s0:
    match:
      macaddress: 52:54:00:cc:dd:10
    set-name: enp1s0
    dhcp4: false
    addresses: [192.168.3.10/24]
    routes:
      - to: default
        via: 192.168.3.1
    nameservers:
      addresses: [8.8.8.8,8.8.4.4]
EOF

# Paso 6: crear VM con virt-install
if ! sudo virsh dominfo ceph-admin >/dev/null 2>&1; then
  sudo virt-install \
    --name ceph-admin \n    --ram 2048 \n    --vcpus 2 \n    --disk path=/var/lib/libvirt/images/ceph-admin.qcow2\,format=qcow2 \n    --network network=net-192-168-3\,model=virtio\,mac=52:54:00:cc:dd:10 \n    --os-variant ubuntu22.04 \n    --cloud-init user-data=/tmp/user-data-ceph-admin.yaml\,network-config=/tmp/network-config-ceph-admin.yaml \n    --noautoconsole \n    --import
else
  echo \[WARN\]\ La\ VM\ ceph-admin\ ya\ existe\ en\ libvirt.\ Se\ omite\ creacion.
fi

