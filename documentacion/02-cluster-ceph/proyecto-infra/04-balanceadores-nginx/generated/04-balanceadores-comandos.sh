#!/usr/bin/env bash
set -euo pipefail

# Archivo generado por create-kvm-vm.sh --comandos
# Ejecuta este archivo cuando quieras crear las VMs manualmente.

# 2026-04-30 23:17:57 ------------------------------------------------
# VM: lb1
# Paso 1: validar imagen base
test -f /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 || { echo '[ERROR] Falta imagen base: /var/lib/libvirt/images/ubuntu-22.04-base.qcow2'; exit 1; }

# Paso 2: generar disco del sistema (si no existe)
if [[ ! -f /var/lib/libvirt/images/lb1.qcow2 ]]; then
  sudo qemu-img create -f qcow2 -b /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 -F qcow2 /var/lib/libvirt/images/lb1.qcow2 20G
fi

# Paso 4: crear cloud-init user-data
cat > /tmp/user-data-lb1.yaml <<'EOF'
#cloud-config
hostname: lb1
manage_etc_hosts: false
timezone: America/El_Salvador
users:
  - name: admin
    primary_group: admin
    sudo: ALL=(ALL) NOPASSWD:ALL
    shell: /bin/bash
    lock_passwd: false
    passwd: $6$8ry.3hGZ.N.hryqp$Mr4VSxRQw97n9k239LF0NEaEUOERPEQtu1NGu0cFPrUPqMOKMe0WsXp.MPa.tiIVlSJJhJJTwIc2s/OD35vkg.
    ssh_authorized_keys:
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
chpasswd:
  expire: false
ssh_pwauth: true
write_files:
  - path: /etc/hosts
    owner: root:root
    permissions: '0644'
    content: |
      127.0.0.1 localhost
      127.0.1.1 lb1
      192.168.3.50 lb1
      192.168.3.51 lb2
      192.168.3.52 app1
      192.168.3.53 app2
      192.168.3.54 app3
      192.168.3.55 dns-1
  - path: /home/admin/.ssh/id_rsa
    owner: admin:admin
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /home/admin/.ssh/id_rsa.pub
    owner: admin:admin
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /home/admin/.ssh/authorized_keys
    owner: admin:admin
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/id_rsa
    owner: root:root
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /root/.ssh/id_rsa.pub
    owner: root:root
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/authorized_keys
    owner: root:root
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a

package_update: true
packages:
  - chrony
  - openssh-server
  - curl
  - qemu-guest-agent
runcmd:
  - timedatectl set-timezone America/El_Salvador || true
  - sed -i '/^pool /d;/^server /d' /etc/chrony/chrony.conf
  - printf 'server ntp.ues.edu.sv iburst\n' >> /etc/chrony/chrony.conf
  - systemctl enable --now chrony
  - systemctl enable --now ssh
  - systemctl enable --now qemu-guest-agent
  - chmod 700 /home/admin/.ssh
  - chmod 600 /home/admin/.ssh/id_rsa /home/admin/.ssh/authorized_keys
  - chmod 644 /home/admin/.ssh/id_rsa.pub
  - chown -R admin:admin /home/admin/.ssh
  - chmod 700 /root/.ssh
  - chmod 600 /root/.ssh/id_rsa /root/.ssh/authorized_keys
  - chmod 644 /root/.ssh/id_rsa.pub

EOF

# Paso 5: crear cloud-init network-config
cat > /tmp/network-config-lb1.yaml <<'EOF'
version: 2
ethernets:
  enp1s0:
    match:
      macaddress: 52:54:00:cc:dd:50
    set-name: enp1s0
    dhcp4: false
    addresses: [192.168.3.50/24]
    routes:
      - to: default
        via: 192.168.3.1
    nameservers:
      addresses: [192.168.3.55,8.8.8.8]
EOF

# Paso 6: crear VM con virt-install
if ! sudo virsh dominfo lb1 >/dev/null 2>&1; then
  sudo virt-install \
    --name lb1 \n    --ram 2048 \n    --vcpus 2 \n    --disk path=/var/lib/libvirt/images/lb1.qcow2\,format=qcow2 \n    --network network=net-192-168-3\,model=virtio\,mac=52:54:00:cc:dd:50 \n    --os-variant ubuntu22.04 \n    --cloud-init user-data=/tmp/user-data-lb1.yaml\,network-config=/tmp/network-config-lb1.yaml \n    --noautoconsole \n    --import
else
  echo \[WARN\]\ La\ VM\ lb1\ ya\ existe\ en\ libvirt.\ Se\ omite\ creacion.
fi

# 2026-04-30 23:17:57 ------------------------------------------------
# VM: lb2
# Paso 1: validar imagen base
test -f /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 || { echo '[ERROR] Falta imagen base: /var/lib/libvirt/images/ubuntu-22.04-base.qcow2'; exit 1; }

# Paso 2: generar disco del sistema (si no existe)
if [[ ! -f /var/lib/libvirt/images/lb2.qcow2 ]]; then
  sudo qemu-img create -f qcow2 -b /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 -F qcow2 /var/lib/libvirt/images/lb2.qcow2 20G
fi

# Paso 4: crear cloud-init user-data
cat > /tmp/user-data-lb2.yaml <<'EOF'
#cloud-config
hostname: lb2
manage_etc_hosts: false
timezone: America/El_Salvador
users:
  - name: admin
    primary_group: admin
    sudo: ALL=(ALL) NOPASSWD:ALL
    shell: /bin/bash
    lock_passwd: false
    passwd: $6$3fkz7a5CLOHAXba4$xZ9/.wa5w0DQ2sYfRvNiR.xZ05tUCTpBRxxdWGeUGFRXL9ppSFrx/JMBS3EgY.etBF22Wcs0oLBTL4YM5SDK1.
    ssh_authorized_keys:
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
chpasswd:
  expire: false
ssh_pwauth: true
write_files:
  - path: /etc/hosts
    owner: root:root
    permissions: '0644'
    content: |
      127.0.0.1 localhost
      127.0.1.1 lb2
      192.168.3.50 lb1
      192.168.3.51 lb2
      192.168.3.52 app1
      192.168.3.53 app2
      192.168.3.54 app3
      192.168.3.55 dns-1
  - path: /home/admin/.ssh/id_rsa
    owner: admin:admin
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /home/admin/.ssh/id_rsa.pub
    owner: admin:admin
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /home/admin/.ssh/authorized_keys
    owner: admin:admin
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/id_rsa
    owner: root:root
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /root/.ssh/id_rsa.pub
    owner: root:root
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/authorized_keys
    owner: root:root
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a

package_update: true
packages:
  - chrony
  - openssh-server
  - curl
  - qemu-guest-agent
runcmd:
  - timedatectl set-timezone America/El_Salvador || true
  - sed -i '/^pool /d;/^server /d' /etc/chrony/chrony.conf
  - printf 'server ntp.ues.edu.sv iburst\n' >> /etc/chrony/chrony.conf
  - systemctl enable --now chrony
  - systemctl enable --now ssh
  - systemctl enable --now qemu-guest-agent
  - chmod 700 /home/admin/.ssh
  - chmod 600 /home/admin/.ssh/id_rsa /home/admin/.ssh/authorized_keys
  - chmod 644 /home/admin/.ssh/id_rsa.pub
  - chown -R admin:admin /home/admin/.ssh
  - chmod 700 /root/.ssh
  - chmod 600 /root/.ssh/id_rsa /root/.ssh/authorized_keys
  - chmod 644 /root/.ssh/id_rsa.pub

EOF

# Paso 5: crear cloud-init network-config
cat > /tmp/network-config-lb2.yaml <<'EOF'
version: 2
ethernets:
  enp1s0:
    match:
      macaddress: 52:54:00:cc:dd:51
    set-name: enp1s0
    dhcp4: false
    addresses: [192.168.3.51/24]
    routes:
      - to: default
        via: 192.168.3.1
    nameservers:
      addresses: [192.168.3.55,8.8.8.8]
EOF

# Paso 6: crear VM con virt-install
if ! sudo virsh dominfo lb2 >/dev/null 2>&1; then
  sudo virt-install \
    --name lb2 \n    --ram 2048 \n    --vcpus 2 \n    --disk path=/var/lib/libvirt/images/lb2.qcow2\,format=qcow2 \n    --network network=net-192-168-3\,model=virtio\,mac=52:54:00:cc:dd:51 \n    --os-variant ubuntu22.04 \n    --cloud-init user-data=/tmp/user-data-lb2.yaml\,network-config=/tmp/network-config-lb2.yaml \n    --noautoconsole \n    --import
else
  echo \[WARN\]\ La\ VM\ lb2\ ya\ existe\ en\ libvirt.\ Se\ omite\ creacion.
fi

# 2026-04-30 23:20:43 ------------------------------------------------
# VM: lb1
# Paso 1: validar imagen base
test -f /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 || { echo '[ERROR] Falta imagen base: /var/lib/libvirt/images/ubuntu-22.04-base.qcow2'; exit 1; }

# Paso 2: generar disco del sistema (si no existe)
if [[ ! -f /var/lib/libvirt/images/lb1.qcow2 ]]; then
  sudo qemu-img create -f qcow2 -b /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 -F qcow2 /var/lib/libvirt/images/lb1.qcow2 20G
fi

# Paso 4: crear cloud-init user-data
cat > /tmp/user-data-lb1.yaml <<'EOF'
#cloud-config
hostname: lb1
manage_etc_hosts: false
timezone: America/El_Salvador
users:
  - name: admin
    primary_group: admin
    sudo: ALL=(ALL) NOPASSWD:ALL
    shell: /bin/bash
    lock_passwd: false
    passwd: $6$vv1mCq9zIyCooO.Y$2ycyTkasAbir3hGOf.eXfv17.wPcv4fvzvazkQ3iy0XJlGtJV7eNo4lIfuUu7mtEGAD/KILtXGiJZsxFB1Xj./
    ssh_authorized_keys:
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
chpasswd:
  expire: false
ssh_pwauth: true
write_files:
  - path: /etc/hosts
    owner: root:root
    permissions: '0644'
    content: |
      127.0.0.1 localhost
      127.0.1.1 lb1
      192.168.3.50 lb1
      192.168.3.51 lb2
      192.168.3.52 app1
      192.168.3.53 app2
      192.168.3.54 app3
      192.168.3.55 dns-1
  - path: /home/admin/.ssh/id_rsa
    owner: admin:admin
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /home/admin/.ssh/id_rsa.pub
    owner: admin:admin
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /home/admin/.ssh/authorized_keys
    owner: admin:admin
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/id_rsa
    owner: root:root
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /root/.ssh/id_rsa.pub
    owner: root:root
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/authorized_keys
    owner: root:root
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a

package_update: true
packages:
  - chrony
  - openssh-server
  - curl
  - qemu-guest-agent
runcmd:
  - timedatectl set-timezone America/El_Salvador || true
  - sed -i '/^pool /d;/^server /d' /etc/chrony/chrony.conf
  - printf 'server ntp.ues.edu.sv iburst\n' >> /etc/chrony/chrony.conf
  - systemctl enable --now chrony
  - systemctl enable --now ssh
  - systemctl enable --now qemu-guest-agent
  - chmod 700 /home/admin/.ssh
  - chmod 600 /home/admin/.ssh/id_rsa /home/admin/.ssh/authorized_keys
  - chmod 644 /home/admin/.ssh/id_rsa.pub
  - chown -R admin:admin /home/admin/.ssh
  - chmod 700 /root/.ssh
  - chmod 600 /root/.ssh/id_rsa /root/.ssh/authorized_keys
  - chmod 644 /root/.ssh/id_rsa.pub

EOF

# Paso 5: crear cloud-init network-config
cat > /tmp/network-config-lb1.yaml <<'EOF'
version: 2
ethernets:
  enp1s0:
    match:
      macaddress: 52:54:00:cc:dd:50
    set-name: enp1s0
    dhcp4: false
    addresses: [192.168.3.50/24]
    routes:
      - to: default
        via: 192.168.3.1
    nameservers:
      addresses: [192.168.3.55,8.8.8.8]
EOF

# Paso 6: crear VM con virt-install
if ! sudo virsh dominfo lb1 >/dev/null 2>&1; then
  sudo virt-install \
    --name lb1 \n    --ram 2048 \n    --vcpus 2 \n    --disk path=/var/lib/libvirt/images/lb1.qcow2\,format=qcow2 \n    --network network=net-192-168-3\,model=virtio\,mac=52:54:00:cc:dd:50 \n    --os-variant ubuntu22.04 \n    --cloud-init user-data=/tmp/user-data-lb1.yaml\,network-config=/tmp/network-config-lb1.yaml \n    --noautoconsole \n    --import
else
  echo \[WARN\]\ La\ VM\ lb1\ ya\ existe\ en\ libvirt.\ Se\ omite\ creacion.
fi

# 2026-04-30 23:20:43 ------------------------------------------------
# VM: lb2
# Paso 1: validar imagen base
test -f /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 || { echo '[ERROR] Falta imagen base: /var/lib/libvirt/images/ubuntu-22.04-base.qcow2'; exit 1; }

# Paso 2: generar disco del sistema (si no existe)
if [[ ! -f /var/lib/libvirt/images/lb2.qcow2 ]]; then
  sudo qemu-img create -f qcow2 -b /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 -F qcow2 /var/lib/libvirt/images/lb2.qcow2 20G
fi

# Paso 4: crear cloud-init user-data
cat > /tmp/user-data-lb2.yaml <<'EOF'
#cloud-config
hostname: lb2
manage_etc_hosts: false
timezone: America/El_Salvador
users:
  - name: admin
    primary_group: admin
    sudo: ALL=(ALL) NOPASSWD:ALL
    shell: /bin/bash
    lock_passwd: false
    passwd: $6$HujINinWXxcpoI.t$YlPyl..5TKcui4RQfp6NyTOdtpnWaWDsmKBPxe4wNFNxF50SyKP2nXTY67etIDkf3WN3LzFZrL2cqReHDumvj/
    ssh_authorized_keys:
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
chpasswd:
  expire: false
ssh_pwauth: true
write_files:
  - path: /etc/hosts
    owner: root:root
    permissions: '0644'
    content: |
      127.0.0.1 localhost
      127.0.1.1 lb2
      192.168.3.50 lb1
      192.168.3.51 lb2
      192.168.3.52 app1
      192.168.3.53 app2
      192.168.3.54 app3
      192.168.3.55 dns-1
  - path: /home/admin/.ssh/id_rsa
    owner: admin:admin
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /home/admin/.ssh/id_rsa.pub
    owner: admin:admin
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /home/admin/.ssh/authorized_keys
    owner: admin:admin
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/id_rsa
    owner: root:root
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /root/.ssh/id_rsa.pub
    owner: root:root
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/authorized_keys
    owner: root:root
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a

package_update: true
packages:
  - chrony
  - openssh-server
  - curl
  - qemu-guest-agent
runcmd:
  - timedatectl set-timezone America/El_Salvador || true
  - sed -i '/^pool /d;/^server /d' /etc/chrony/chrony.conf
  - printf 'server ntp.ues.edu.sv iburst\n' >> /etc/chrony/chrony.conf
  - systemctl enable --now chrony
  - systemctl enable --now ssh
  - systemctl enable --now qemu-guest-agent
  - chmod 700 /home/admin/.ssh
  - chmod 600 /home/admin/.ssh/id_rsa /home/admin/.ssh/authorized_keys
  - chmod 644 /home/admin/.ssh/id_rsa.pub
  - chown -R admin:admin /home/admin/.ssh
  - chmod 700 /root/.ssh
  - chmod 600 /root/.ssh/id_rsa /root/.ssh/authorized_keys
  - chmod 644 /root/.ssh/id_rsa.pub

EOF

# Paso 5: crear cloud-init network-config
cat > /tmp/network-config-lb2.yaml <<'EOF'
version: 2
ethernets:
  enp1s0:
    match:
      macaddress: 52:54:00:cc:dd:51
    set-name: enp1s0
    dhcp4: false
    addresses: [192.168.3.51/24]
    routes:
      - to: default
        via: 192.168.3.1
    nameservers:
      addresses: [192.168.3.55,8.8.8.8]
EOF

# Paso 6: crear VM con virt-install
if ! sudo virsh dominfo lb2 >/dev/null 2>&1; then
  sudo virt-install \
    --name lb2 \n    --ram 2048 \n    --vcpus 2 \n    --disk path=/var/lib/libvirt/images/lb2.qcow2\,format=qcow2 \n    --network network=net-192-168-3\,model=virtio\,mac=52:54:00:cc:dd:51 \n    --os-variant ubuntu22.04 \n    --cloud-init user-data=/tmp/user-data-lb2.yaml\,network-config=/tmp/network-config-lb2.yaml \n    --noautoconsole \n    --import
else
  echo \[WARN\]\ La\ VM\ lb2\ ya\ existe\ en\ libvirt.\ Se\ omite\ creacion.
fi

# 2026-04-30 23:50:50 ------------------------------------------------
# VM: lb1
# Paso 1: validar imagen base
test -f /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 || { echo '[ERROR] Falta imagen base: /var/lib/libvirt/images/ubuntu-22.04-base.qcow2'; exit 1; }

# Paso 2: generar disco del sistema (si no existe)
if [[ ! -f /var/lib/libvirt/images/lb1.qcow2 ]]; then
  sudo qemu-img create -f qcow2 -b /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 -F qcow2 /var/lib/libvirt/images/lb1.qcow2 20G
fi

# Paso 4: crear cloud-init user-data
cat > /tmp/user-data-lb1.yaml <<'EOF'
#cloud-config
hostname: lb1
manage_etc_hosts: false
timezone: America/El_Salvador
users:
  - name: admin
    primary_group: admin
    sudo: ALL=(ALL) NOPASSWD:ALL
    shell: /bin/bash
    lock_passwd: false
    passwd: $6$iw79Mq7/V/qglNRh$vWRk5NLNgB/WkVJ/NNZOnSlagnZyerMal1UGFM78SF.cMLBBnv19hq1.UwDk51LGgrzmGlcRDLEYp/j.4TQUp/
    ssh_authorized_keys:
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
chpasswd:
  expire: false
ssh_pwauth: true
write_files:
  - path: /etc/hosts
    owner: root:root
    permissions: '0644'
    content: |
      127.0.0.1 localhost
      127.0.1.1 lb1
      192.168.3.50 lb1
      192.168.3.51 lb2
      192.168.3.52 app1
      192.168.3.53 app2
      192.168.3.54 app3
      192.168.3.55 dns-1
  - path: /home/admin/.ssh/id_rsa
    owner: admin:admin
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /home/admin/.ssh/id_rsa.pub
    owner: admin:admin
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /home/admin/.ssh/authorized_keys
    owner: admin:admin
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/id_rsa
    owner: root:root
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /root/.ssh/id_rsa.pub
    owner: root:root
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/authorized_keys
    owner: root:root
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a

package_update: true
packages:
  - chrony
  - openssh-server
  - curl
  - qemu-guest-agent
runcmd:
  - timedatectl set-timezone America/El_Salvador || true
  - sed -i '/^pool /d;/^server /d' /etc/chrony/chrony.conf
  - printf 'server ntp.ues.edu.sv iburst\n' >> /etc/chrony/chrony.conf
  - systemctl enable --now chrony
  - systemctl enable --now ssh
  - systemctl enable --now qemu-guest-agent
  - chmod 700 /home/admin/.ssh
  - chmod 600 /home/admin/.ssh/id_rsa /home/admin/.ssh/authorized_keys
  - chmod 644 /home/admin/.ssh/id_rsa.pub
  - chown -R admin:admin /home/admin/.ssh
  - chmod 700 /root/.ssh
  - chmod 600 /root/.ssh/id_rsa /root/.ssh/authorized_keys
  - chmod 644 /root/.ssh/id_rsa.pub

EOF

# Paso 5: crear cloud-init network-config
cat > /tmp/network-config-lb1.yaml <<'EOF'
version: 2
ethernets:
  enp1s0:
    match:
      macaddress: 52:54:00:cc:dd:50
    set-name: enp1s0
    dhcp4: false
    addresses: [192.168.3.50/24]
    routes:
      - to: default
        via: 192.168.3.1
    nameservers:
      addresses: [192.168.3.55,8.8.8.8]
EOF

# Paso 6: crear VM con virt-install
if ! sudo virsh dominfo lb1 >/dev/null 2>&1; then
  sudo virt-install \
    --name lb1 \n    --ram 2048 \n    --vcpus 2 \n    --disk path=/var/lib/libvirt/images/lb1.qcow2\,format=qcow2 \n    --network network=net-192-168-3\,model=virtio\,mac=52:54:00:cc:dd:50 \n    --os-variant ubuntu22.04 \n    --cloud-init user-data=/tmp/user-data-lb1.yaml\,network-config=/tmp/network-config-lb1.yaml \n    --noautoconsole \n    --import
else
  echo \[WARN\]\ La\ VM\ lb1\ ya\ existe\ en\ libvirt.\ Se\ omite\ creacion.
fi

# 2026-04-30 23:50:50 ------------------------------------------------
# VM: lb2
# Paso 1: validar imagen base
test -f /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 || { echo '[ERROR] Falta imagen base: /var/lib/libvirt/images/ubuntu-22.04-base.qcow2'; exit 1; }

# Paso 2: generar disco del sistema (si no existe)
if [[ ! -f /var/lib/libvirt/images/lb2.qcow2 ]]; then
  sudo qemu-img create -f qcow2 -b /var/lib/libvirt/images/ubuntu-22.04-base.qcow2 -F qcow2 /var/lib/libvirt/images/lb2.qcow2 20G
fi

# Paso 4: crear cloud-init user-data
cat > /tmp/user-data-lb2.yaml <<'EOF'
#cloud-config
hostname: lb2
manage_etc_hosts: false
timezone: America/El_Salvador
users:
  - name: admin
    primary_group: admin
    sudo: ALL=(ALL) NOPASSWD:ALL
    shell: /bin/bash
    lock_passwd: false
    passwd: $6$B5qfpmX9R5aTqN6T$1.IRN1wkaRh8KxbMifCMgtspvzeLysmXwFp2yy5Wsz0QUmwXC/onkAsk4FmqH63Gdtxte7bzQm7lFo2Crt5Vl0
    ssh_authorized_keys:
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      - ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
chpasswd:
  expire: false
ssh_pwauth: true
write_files:
  - path: /etc/hosts
    owner: root:root
    permissions: '0644'
    content: |
      127.0.0.1 localhost
      127.0.1.1 lb2
      192.168.3.50 lb1
      192.168.3.51 lb2
      192.168.3.52 app1
      192.168.3.53 app2
      192.168.3.54 app3
      192.168.3.55 dns-1
  - path: /home/admin/.ssh/id_rsa
    owner: admin:admin
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /home/admin/.ssh/id_rsa.pub
    owner: admin:admin
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /home/admin/.ssh/authorized_keys
    owner: admin:admin
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/id_rsa
    owner: root:root
    permissions: '0600'
    content: |
      -----BEGIN OPENSSH PRIVATE KEY-----
      
      -----END OPENSSH PRIVATE KEY-----
  - path: /root/.ssh/id_rsa.pub
    owner: root:root
    permissions: '0644'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
  - path: /root/.ssh/authorized_keys
    owner: root:root
    permissions: '0600'
    content: |
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a
      ssh-rsa  uceda@uceda-ThinkPad-L15-Gen-2a

package_update: true
packages:
  - chrony
  - openssh-server
  - curl
  - qemu-guest-agent
runcmd:
  - timedatectl set-timezone America/El_Salvador || true
  - sed -i '/^pool /d;/^server /d' /etc/chrony/chrony.conf
  - printf 'server ntp.ues.edu.sv iburst\n' >> /etc/chrony/chrony.conf
  - systemctl enable --now chrony
  - systemctl enable --now ssh
  - systemctl enable --now qemu-guest-agent
  - chmod 700 /home/admin/.ssh
  - chmod 600 /home/admin/.ssh/id_rsa /home/admin/.ssh/authorized_keys
  - chmod 644 /home/admin/.ssh/id_rsa.pub
  - chown -R admin:admin /home/admin/.ssh
  - chmod 700 /root/.ssh
  - chmod 600 /root/.ssh/id_rsa /root/.ssh/authorized_keys
  - chmod 644 /root/.ssh/id_rsa.pub

EOF

# Paso 5: crear cloud-init network-config
cat > /tmp/network-config-lb2.yaml <<'EOF'
version: 2
ethernets:
  enp1s0:
    match:
      macaddress: 52:54:00:cc:dd:51
    set-name: enp1s0
    dhcp4: false
    addresses: [192.168.3.51/24]
    routes:
      - to: default
        via: 192.168.3.1
    nameservers:
      addresses: [192.168.3.55,8.8.8.8]
EOF

# Paso 6: crear VM con virt-install
if ! sudo virsh dominfo lb2 >/dev/null 2>&1; then
  sudo virt-install \
    --name lb2 \n    --ram 2048 \n    --vcpus 2 \n    --disk path=/var/lib/libvirt/images/lb2.qcow2\,format=qcow2 \n    --network network=net-192-168-3\,model=virtio\,mac=52:54:00:cc:dd:51 \n    --os-variant ubuntu22.04 \n    --cloud-init user-data=/tmp/user-data-lb2.yaml\,network-config=/tmp/network-config-lb2.yaml \n    --noautoconsole \n    --import
else
  echo \[WARN\]\ La\ VM\ lb2\ ya\ existe\ en\ libvirt.\ Se\ omite\ creacion.
fi

